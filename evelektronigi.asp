...
<% Dim strEV
Dim objEV
strEV = "SELECT * FROM urun INNER JOIN img ON urun.img_id=img.id WHERE kategori_id=4"
Set objEV = Baglanti.Execute(strEV)
%>
...
If objEV.EOF Then
  Response.Write("No items found")
Else
  Do While Not objEV.EOF
  Response.Write("<div class='col-sm-4'>")
   Response.Write("<div class='card h-100'>") 
   response.write("<img class='card-img-top' src='"& objEV.Fields("path").value &"'>")  
   response.write("<div class='card-body h-100'>")
   response.write("<h5 class='card-title'>" & objEV.Fields("adi").value & "</h5>")
   response.write("<p class='price'>" & objEV.Fields("fiyat").value & " TL</p>")
