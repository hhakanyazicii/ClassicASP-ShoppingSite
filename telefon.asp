...
<% Dim strTEL
Dim objTEL
strTEL = "SELECT * FROM urun INNER JOIN img ON urun.img_id=img.id WHERE kategori_id=2"
Set objTEL = Baglanti.Execute(strTEL)
%>
...
If objTEL.EOF Then
  Response.Write("No items found")
Else
  Do While Not objTEL.EOF
  Response.Write("<div class='col-sm-4'>")
   Response.Write("<div class='card h-100'>") 
   response.write("<img class='card-img-top' src='"& objTEL.Fields("path").value &"'>")  
   response.write("<div class='card-body h-100'>")
   response.write("<h5 class='card-title'>" & objTEL.Fields("adi").value & "</h5>")
   response.write("<p class='price'>" & objTEL.Fields("fiyat").value & " TL</p>")
