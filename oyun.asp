...
<% Dim strOyun
Dim objOyun
strOyun = "SELECT * FROM urun INNER JOIN img ON urun.img_id=img.id WHERE kategori_id=5"
Set objOyun = Baglanti.Execute(strOyun)
%>
...
If objOyun.EOF Then
  Response.Write("No items found")
Else
  Do While Not objOyun.EOF
  Response.Write("<div class='col-sm-4'>")
   Response.Write("<div class='card h-100'>") 
   response.write("<img class='card-img-top' src='"& objOyun.Fields("path").value &"'>")  
   response.write("<div class='card-body h-100'>")
   response.write("<h5 class='card-title'>" & objOyun.Fields("adi").value & "</h5>")
   response.write("<p class='price'>" & objOyun.Fields("fiyat").value & " TL</p>")
