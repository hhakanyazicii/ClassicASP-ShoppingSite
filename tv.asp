...
<% Dim strTV
Dim objTV
strTV = "SELECT * FROM urun INNER JOIN img ON urun.img_id=img.id WHERE kategori_id=3"
Set objTV = Baglanti.Execute(strTV)
%>
...
If objTV.EOF Then
  Response.Write("No items found")
Else
  Do While Not objTV.EOF
  Response.Write("<div class='col-sm-4'>")
   Response.Write("<div class='card h-100'>") 
   response.write("<img class='card-img-top' src='"& objTV.Fields("path").value &"'>")  
   response.write("<div class='card-body h-100'>")
   response.write("<h5 class='card-title'>" & objTV.Fields("adi").value & "</h5>")
   response.write("<p class='price'>" & objTV.Fields("fiyat").value & " TL</p>")
