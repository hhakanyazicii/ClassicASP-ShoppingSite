...
<% Dim strPC
Dim objPC
strPC = "SELECT * FROM urun INNER JOIN img ON urun.img_id=img.id WHERE kategori_id=1 "
Set objPC = Baglanti.Execute(strPC)
%>
...
If objPC.EOF Then
  Response.Write("No items found")
Else
  Do While Not objPC.EOF
  Response.Write("<div class='col-sm-4'>")
   Response.Write("<div class='card h-100'>") 
   response.write("<img class='card-img-top' src='"& objPC.Fields("path").value &"'>")  
   response.write("<div class='card-body h-100'>")
   response.write("<h5 class='card-title'>" & objPC.Fields("adi").value & "</h5>")
   response.write("<p class='price'>" & objPC.Fields("fiyat").value & " TL</p>")
